...
ProjectBoard 是面向人类与 Agent 协作的安全任务队列。人创建任务并定义结果，人类成员或 Agent 在同一条连续对话中完成讨论、执行和验收，Runner 负责本地 Codex 调用、Git、验证、租约与结构化结果回写。

产品不是通用 Jira，也不在服务端运行模型或保存 Git 凭据。服务端负责状态、权限、审计和协作；代码执行只发生在登记过的 Runner 主机。

### 1.1 完成标准

- 任务严格使用 `todo → discussion → execution → acceptance → completed` 主流程，并支持从任意未完成阶段进入 `abandoned`。
- 阻塞使用 `blocked_at + blocked_reason` 附加标记，不改变任务主阶段。
- 分配执行者后，待办任务进入讨论；任意未完成阶段都能更换执行者，旧租约立即终止，全部历史保留。
...
- Git commit message 包含完整任务 ID 时，提交自动成为对话中的系统证据事件；单个提交不能触发完成。
...
- `administrator`：系统级角色；管理用户、项目、仓库执行配置、成员、Agent、Token、全部任务和审计。
- `developer`：项目级角色；创建和修改任务、参与讨论、人工执行、分配执行者、提交结果、验收和放弃任务。
- `viewer`：项目级只读角色。
...
- 同一 Agent 可以有多个待接受任务，但同时只能持有一个活跃租约。
...
- 新任务默认 `todo`，使用项目默认模式和默认目标分支。
- 阻塞是附加标记；阻塞任务仍保持原 `stage`，解除阻塞只清空阻塞字段。
- `completed` 和 `abandoned` 是终态。
- 目标分支在开始讨论前可直接修改；开始后修改属于范围变化，递增版本、终止租约并返回讨论。
- Git、验证和结论数据不重复存入任务主表。
...
- 所属任务、发生时的阶段、作者类型/身份、时间、不可变载荷和关联版本。
...
- 产物包括摘要、commit hashes、改动文件、验证结果、剩余风险、附件和发现任务。
...
- `completed` 不允许重新打开；后续工作创建新任务并建立来源关系。
...
- 阻塞必须提供原因，可选关联依赖或发现任务。
- Runner 阻塞时结束当前活跃尝试并释放租约，但任务主阶段不变。
- 解除阻塞不创建新阶段；重新工作时创建新的执行尝试或继续讨论。
- 放弃必须提供原因，终止租约并写入终态事件；历史和附件不删除。

## 6. 父子任务与依赖

- 任务支持父任务与直接子任务两层；子任务不能继续拆分。
- 讨论结论可以附带一次结构化拆分提案；全部子任务与依赖必须原子创建。
- 拆分提案至少 2 项、最多 20 项，校验唯一 key、无环、同父任务引用和非空验收条件。
- 子任务创建为 `todo`，继承目标分支与自动模式。
- 父任务拆分后保持 `execution`，派生状态显示“等待子项”，不进入 Agent 队列。
- 只有依赖完成且未阻塞的叶子任务可被领取。
- 全部直接子任务完成后，父任务进入 `acceptance`；通过后才完成。
- 已产生对话、尝试、Git 或验收记录的子任务不能删除，只能放弃。

## 7. 租约与并发

- Agent 接受指派或自领后创建 `active_lease`；保留指派使用 `reserved`。
- 同一任务同一时刻最多一个执行租约，同一 Agent 最多一个活跃租约。
...
- 只接受可从当前项目任务分支到达、且消息包含完整任务 ID 的 commit。
...
- 点击右上角头像进入个人账号管理。
- 头像左侧提供语言切换按钮，支持 `zh-CN` 与 `en`，默认 `zh-CN`；切换语言不改变当前项目、页面、筛选、已选任务或抽屉上下文。
...
- 新任务默认待办，可选负责人、目标分支、优先级、`discussion_mode` 和 `execution_mode`；验收模式可配置为 human、independent_agent 或 auto。
- 分配负责人后进入讨论；未分配时保持待办。
...
- 项目名称、任务标题、描述、普通消息、附件名称和其他用户创建内容保持原文，不执行机器翻译。
- 动态渲染的队列行、完整任务、管理页和抽屉必须继承当前语言，不得闪回默认文案或重置操作上下文。
...
- 人类活动可包含任务操作、成员关系、登录和账户事件；Agent 活动可包含领取/租约、心跳、消息、执行结果、Git 证据和验证事件。
...
所有写操作必须带 `requestId`；修改现有任务还必须带 `expectedVersion`。服务端不能信任客户端传入的项目、阶段、执行者或 Git 归属。

## 12. 持久化与事务约束

- SQLite 开启 WAL、foreign_keys、busy_timeout 和定期备份。
- 任务编号对 `(project_id, number)` 唯一。
- 依赖对、项目成员、Agent 项目授权和附件哈希建立唯一或必要索引。
- Git 证据对 `(repository_id, commit_sha)` 唯一。
- 同一任务只能有一个活跃执行租约；同一 Agent 只能有一个活跃租约。
...
- 人类/Runner 同步授权、repository/SHA 幂等、完整任务 ID 关联、installation 仓库移除和权限降级均有测试。
- 临时远端覆盖 main、develop、release/* 基线和任务分支 fast-forward push。
- 完整任务 ID 边界匹配；错误仓库、错误分支、短 ID 和未知 commit 拒绝。
...
- 默认加载中文；中英文连续切换不会改变项目、页面、队列筛选、选中任务、滚动位置或抽屉内容。
