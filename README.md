...
- 每个项目有一棵共享知识树，支持 Markdown、子节点、关键词搜索、版本历史和恢复；知识内容直接保存在节点中，不引用外部文件。
...
默认仅监听本机回环地址，Web 地址为 `http://127.0.0.1:3333`，数据目录为 `./data`。生产环境应通过 TLS 反向代理提供 HTTPS。
...
管理员创建本地 Codex Agent，并设置并发容量、单次需求超时、接取标签与拒绝标签。没有匹配 Agent 时，需求保持排队。原始 JSONL 会在执行期间持续写入；管理员可在“设置 → 本地 Agent”中查看实时输出。运行中的任务被标记为阻塞时，只终止该任务的 Codex 进程树；连续 10 分钟没有 CLI 输出或活动时，执行会自动停止并释放容量。
...
知识整理 Agent 只能提交结构化节点操作，服务端在单个事务中校验锁、版本和项目归属。知识节点不提供附件上传或外部文件引用。
