...
当前实现只操作服务器本地 Git 仓库，并由服务进程直接调用本地 `codex` CLI；不依赖 SSH、外部 Runner、外部 MCP 服务、向量数据库、图数据库或 Git 托管平台授权。普通 Agent 请求会临时启用内置的只读知识 MCP。
...
- Agent：本地执行身份、并发容量、单轮超时、接取/拒绝标签、实时输出、暂停、继续、取消、重试和计划批准。
- 知识库：项目级树状节点、Markdown、摘要与触发描述、FTS5 全文检索、按需 Agent 读取、移动、Agent 锁、完整修订历史和版本恢复。
...
每次计划、执行、计划批准、重试、返工、合并或知识整理都是独立的 Agent 需求，并使用新的 Codex 会话。原始 JSONL、最终消息、错误和请求谱系会保留。没有符合标签或容量条件的 Agent 时，需求保持排队。

普通任务只在初始上下文中接收知识节点路径、摘要和“何时使用”描述；目录预算约为 8,000 字节，超出部分仍可搜索。当任务依赖项目特有的约定、架构决定或运行手册时，Codex 可主动调用内置的 `knowledge_search`，再用 `knowledge_read` 展开命中节点。该 STDIO MCP 进程只读并绑定当前项目；项目 ID 不是模型可修改的工具参数。知识整理需求仍读取完整快照，以便提交完整的结构化节点操作。
...
SQLite 使用 WAL，当前 schema 为 v10。首次打开 v9 数据库时会事务性增加知识触发元数据和 FTS5 索引并迁移到 v10；其他 schema 会被拒绝。升级前仍应先备份。
