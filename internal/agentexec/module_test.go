...
func TestLocalAgentPausesAfterInitialPlan(t *testing.T) {
	...
	if err = os.MkdirAll(filepath.Join(dir, "workspaces", strings.ToLower(request.ID), ".git"), 0o700); err != nil {
		t.Fatal(err)
	}
	...
	case call := <-runner.calls:
		...
		if !strings.Contains(call.Prompt, "already-created ProjectBoard branch") {
	...
	execution, err := requests.Create(t.Context(), agentrequest.CreateInput{ProjectID: "p", Kind: "task_execution", SourceWorkItemID: item.ID, Title: "Execute task", PromptMarkdown: plan, CreatedByType: "human", CreatedByID: "u"})
	if err != nil {
		t.Fatal(err)
	}
	if err = os.MkdirAll(filepath.Join(dir, "workspaces", strings.ToLower(execution.ID), ".git"), 0o700); err != nil {
		t.Fatal(err)
	}
	module.Wake()
	select {
...
func TestKnowledgeRequestAppliesStructuredOperations(t *testing.T) {
	...
	stamp := time.Now().UTC().Format(time.RFC3339Nano)
	_, err = database.DB.Exec("INSERT INTO projects(id,project_key,name,repository_url,knowledge_compaction_days,knowledge_compaction_request_count,created_at,updated_at) VALUES('p','KB','Knowledge','',0,0,?,?)", stamp, stamp)
	if err != nil {
		t.Fatal(err)
	}
