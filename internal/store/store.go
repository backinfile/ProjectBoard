...
func migrate(db *sql.DB, version int) error {
	...
	if version < 5 {
		...
		statements := []string{
			"ALTER TABLE agent_project_grants ADD COLUMN allow_unassigned_claim INTEGER NOT NULL DEFAULT 0",
			"ALTER TABLE runner_runs RENAME TO agent_executions",
			"CREATE TABLE agent_ssh_keys(id TEXT PRIMARY KEY,agent_id TEXT NOT NULL REFERENCES agents(id) ON DELETE CASCADE,label TEXT NOT NULL,public_key TEXT NOT NULL,fingerprint TEXT NOT NULL UNIQUE,created_at TEXT NOT NULL,last_used_at TEXT,revoked_at TEXT)",
			"CREATE TABLE agent_ssh_sessions(id TEXT PRIMARY KEY,agent_id TEXT NOT NULL REFERENCES agents(id),key_id TEXT NOT NULL REFERENCES agent_ssh_keys(id),remote_addr TEXT NOT NULL,client_version TEXT NOT NULL,connected_at TEXT NOT NULL,last_seen_at TEXT NOT NULL,disconnected_at TEXT,disconnect_reason TEXT)",
			"CREATE INDEX agent_ssh_sessions_agent_time ON agent_ssh_sessions(agent_id,connected_at DESC)",
			"CREATE TABLE agent_environment_steps(id TEXT PRIMARY KEY,execution_id TEXT NOT NULL REFERENCES agent_executions(id),command TEXT NOT NULL,exit_code INTEGER NOT NULL,duration_ms INTEGER NOT NULL,summary TEXT NOT NULL,created_at TEXT NOT NULL)",
			"UPDATE leases SET released_at=CURRENT_TIMESTAMP,release_reason='auth_migration' WHERE released_at IS NULL",
			"UPDATE agent_executions SET ended_at=CURRENT_TIMESTAMP,end_reason='auth_migration' WHERE ended_at IS NULL",
			"UPDATE assignments SET state='reserved' WHERE id IN (SELECT assignment_id FROM leases WHERE release_reason='auth_migration')",
			"UPDATE work_items SET assignment_state='reserved',version=version+1,updated_at=CURRENT_TIMESTAMP WHERE id IN (SELECT work_item_id FROM leases WHERE release_reason='auth_migration')",
			"INSERT INTO activity_events(id,project_id,actor_type,actor_id,event_type,object_type,object_id,source,payload_json,created_at) SELECT lower(hex(randomblob(18))),w.project_id,'system',NULL,'auth_migration','work_item',w.id,'migration','{}',CURRENT_TIMESTAMP FROM work_items w WHERE w.id IN (SELECT work_item_id FROM leases WHERE release_reason='auth_migration')",
			"DELETE FROM project_repository_grants WHERE authorization_id IN (SELECT id FROM provider_authorizations WHERE provider='gitlab')",
			"DELETE FROM provider_authorization_repositories WHERE authorization_id IN (SELECT id FROM provider_authorizations WHERE provider='gitlab')",
			"DELETE FROM provider_authorization_metadata WHERE authorization_id IN (SELECT id FROM provider_authorizations WHERE provider='gitlab')",
			"DELETE FROM git_connection_flows WHERE provider='gitlab'",
			"DELETE FROM provider_authorizations WHERE provider='gitlab'",
			"DELETE FROM git_provider_settings WHERE provider='gitlab'",
			"DROP TABLE runner_status",
			"DROP TABLE runner_repository_mappings",
			"DROP TABLE runner_devices",
			"DROP TABLE runner_pairing_codes",
			"DROP TABLE agent_key_settings",
...
INSERT OR IGNORE INTO schema_metadata(id,version,created_at) VALUES(1,5,CURRENT_TIMESTAMP);
...
CREATE TABLE IF NOT EXISTS agent_project_grants(agent_id TEXT NOT NULL REFERENCES agents(id), project_id TEXT NOT NULL REFERENCES projects(id), allow_unassigned_claim INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL, PRIMARY KEY(agent_id,project_id));
CREATE TABLE IF NOT EXISTS agent_ssh_keys(id TEXT PRIMARY KEY,agent_id TEXT NOT NULL REFERENCES agents(id) ON DELETE CASCADE,label TEXT NOT NULL,public_key TEXT NOT NULL,fingerprint TEXT NOT NULL UNIQUE,created_at TEXT NOT NULL,last_used_at TEXT,revoked_at TEXT);
CREATE TABLE IF NOT EXISTS agent_ssh_sessions(id TEXT PRIMARY KEY,agent_id TEXT NOT NULL REFERENCES agents(id),key_id TEXT NOT NULL REFERENCES agent_ssh_keys(id),remote_addr TEXT NOT NULL,client_version TEXT NOT NULL,connected_at TEXT NOT NULL,last_seen_at TEXT NOT NULL,disconnected_at TEXT,disconnect_reason TEXT);
CREATE INDEX IF NOT EXISTS agent_ssh_sessions_agent_time ON agent_ssh_sessions(agent_id,connected_at DESC);
...
CREATE TABLE IF NOT EXISTS agent_executions(id TEXT PRIMARY KEY, work_item_id TEXT NOT NULL REFERENCES work_items(id), agent_id TEXT NOT NULL REFERENCES agents(id), lease_id TEXT NOT NULL REFERENCES leases(id), state TEXT NOT NULL, created_at TEXT NOT NULL, ended_at TEXT, end_reason TEXT);
CREATE TABLE IF NOT EXISTS agent_environment_steps(id TEXT PRIMARY KEY,execution_id TEXT NOT NULL REFERENCES agent_executions(id),command TEXT NOT NULL,exit_code INTEGER NOT NULL,duration_ms INTEGER NOT NULL,summary TEXT NOT NULL,created_at TEXT NOT NULL);
