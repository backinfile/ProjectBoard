...
func TestCompactionSchedulesAtConfiguredCountWithoutDuplicates(t *testing.T) {
	...
	_, err = database.DB.Exec(`INSERT INTO projects(id,project_key,name,repository_url,knowledge_compaction_days,knowledge_compaction_request_count,created_at,updated_at) VALUES('p','KB','Knowledge','',0,2,'now','now')`)
	if err != nil {
		t.Fatal(err)
	}
	module := New(database)
...
func TestApprovingPlanCreatesFreshExecutionRequest(t *testing.T) {
	...
	defer database.Close()
	_, err = database.DB.Exec(`INSERT INTO projects(id,project_key,name,repository_url,created_at,updated_at) VALUES('p','PLAN','Plan','', 'now','now');
...
func TestTaskCannotHaveTwoActiveAgentRequests(t *testing.T) {
	...
	defer database.Close()
	_, err = database.DB.Exec(`INSERT INTO projects(id,project_key,name,repository_url,created_at,updated_at) VALUES('p','ONE','One','', 'now','now');
