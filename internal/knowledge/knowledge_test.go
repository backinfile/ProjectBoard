...
func TestAgentLocksVersioningAndRevisionRestore(t *testing.T) {
	...
	_, err = database.DB.Exec(`INSERT INTO projects(id,project_key,name,repository_url,created_at,updated_at) VALUES('project-1','know','Knowledge','https://example.com/know.git','now','now')`)
	if err != nil {
		t.Fatal(err)
	}
	module := New(database)
...
func TestAgentOperationsAreAtomicAndCannotDeleteLockedSubtrees(t *testing.T) {
	...
	defer database.Close()
	_, err = database.DB.Exec(`INSERT INTO projects(id,project_key,name,repository_url,created_at,updated_at) VALUES('project-1','atomic','Atomic','https://example.com/atomic.git','now','now')`)
	if err != nil {
		t.Fatal(err)
	}
	module := New(database)
