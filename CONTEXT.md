...
- Scheduling: an event-woken loop with polling fallback applies per-Agent capacity and accept/reject tag filters. Each Agent can override the Codex model and reasoning effort.
...
- Knowledge: one versioned Markdown tree per project, with keyword search, revisions, restore, optimistic writes, and node-local Agent locks.
...
- Persistence: SQLite WAL schema v10 plus file attachments and temporary Agent workspaces under the data directory. Schema v9 is migrated in place; other versions are rejected.
